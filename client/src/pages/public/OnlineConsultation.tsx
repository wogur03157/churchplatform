...
export default function OnlineConsultation() {
  ...
  const { data: pageData } = useQuery({
    queryKey: ["content-pages", "public", "canaan", "online-consultation"],
    queryFn: () => api.get<any>("/content-pages/public/canaan/online-consultation"),
  });

  const { data: fields } = useQuery({
    queryKey: ["form-fields", "active", "consultation"],
    queryFn: () => api.get<any[]>("/form-fields?activeOnly=true&formType=consultation"),
  });

  const submitMutation = useMutation({
    mutationFn: (fieldData: Record<string, string>) =>
      api.post("/form-submissions", { formType: "consultation", fieldData }),
    ...
    return (
      <PublicPageLayout>
        <div className="container py-24 max-w-lg text-center">
          ...
          <h2 className="text-2xl font-bold mb-3">상담 신청이 완료되었습니다!</h2>
          <p className="text-muted-foreground">담당 사역자가 2~3일 이내에 연락드리겠습니다.</p>
        </div>
      </PublicPageLayout>
    );
  }
  ...
  return (
    <PublicPageLayout>
      <div className="container py-16 max-w-4xl">
        <h1 className="text-4xl font-bold mb-4">{pageData?.title ?? "온라인 상담 신청"}</h1>
        <p className="text-muted-foreground text-lg mb-12">편하게 상담을 신청해 주세요</p>
