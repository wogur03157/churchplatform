...
- [ ] 홈 화면 colSpan 가로 분할 미반영 — PublicView `flex flex-col` → `grid grid-cols-3` 전환 (an 브랜치 충돌 우려로 보류)
...
### 🔴 메뉴 커스텀 (교회별 소메뉴 이름 설정)
> 레이아웃 개편과 연관성 높음, 헤더 구조 변경 필요
- [ ] 소메뉴 이름 교회별 설정 가능하도록 구조 설계
- [ ] 어드민에서 메뉴 이름 편집 UI 구현
- [ ] 공개 헤더에서 커스텀 메뉴명 반영
