 ...
 *
 * - 키: FINANCE_DATA_KEY 환경변수 (임의 문자열 → SHA-256으로 32바이트 파생)
...
function deriveKey(): Buffer | null {
  const raw = process.env.FINANCE_DATA_KEY;
  if (!raw) return null;
...
function warnOnce(): void {
  ...
  warned = true;
  console.warn(
    "[finance] FINANCE_DATA_KEY가 없어 개인정보 컬럼이 평문으로 저장됩니다 — 운영 환경에서는 반드시 설정하세요"
  );
}
...
export function decryptValue(stored: string): string {
  ...
  const key = deriveKey();
  if (!key) return "[암호화됨 — FINANCE_DATA_KEY 필요]";
