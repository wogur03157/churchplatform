...
> 상태: **백엔드 MVP 구현 완료** (권한·CRUD·엑셀·조직·출석·새가족·대시보드) + 교인 목록 어드민 화면 (2026-07-12)
...
| 3 | ✅ 엑셀 임포트/익스포트 | 헤더 자동매핑·중복검사·행별 오류 리포트 |
| 4 | ✅(API) 조직(member_groups) 트리+배정 | 조직도 화면 미구현 |
| 5 | ✅(API) 출석 세션/기록(upsert)+통계 | 체크 화면 미구현 |
| 6 | ✅(API) 새가족 단계/보드/이동 | 칸반 화면·form-fields 연동 미구현 |
| 7 | 심방 + 목양 메모 (권한 분리) | |
| 8 | ✅ 대시보드 API + 장기결석 감지 + 교인 목록 화면(/admin/members) | 요약카드·검색·CRUD·엑셀 버튼 포함 |
